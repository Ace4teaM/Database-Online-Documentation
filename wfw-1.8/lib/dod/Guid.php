...
class GuidMgr
{
     ...
     * @param $inst Entity instance (Guid)
     * @param $doc Parent document
     * @return New element node
     */
    public static function toXML(&$inst,$doc) {
        $node = $doc->createElement(strtolower("Guid"));
        
        $node->appendChild($doc->createTextElement("guid_id",$inst->guidId));
        $node->appendChild($doc->createTextElement("guid_value",$inst->guidValue));       
    ...
    */
    public static function getAll(&$list,$cond,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from guid where $cond";
      ...
      //extrait les instances
       ...
       while( $result->seek($i,iDatabaseQuery::Origin) ){
        $inst = new Guid();
        GuidMgr::bindResult($inst,$result);
    ...
    /*
      ...
      @param $inst Guid instance pointer to initialize
      @param $cond SQL Select condition
      @param $db iDataBase derived instance
    */
    public static function bindResult(&$inst,$result){
          $inst->guidId = $result->fetchValue("guid_id");
          $inst->guidValue = $result->fetchValue("guid_value");          
    ...
    public static function get(&$inst,$cond,$db=null){
      ...
      //execute la requete
       ...
       if($db->execute($query,$result)){
          ...
          return GuidMgr::bindResult($inst,$result);
       }
       return false;
    }
    ...
    */
    public static function getById(&$inst,$id,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from guid where guid_id=".$db->parseValue($id);
       if($db->execute($query,$result)){
            $inst = new Guid();
             if(!$result->rowCount())
                 return RESULT(cResult::Failed,iDatabaseQuery::EmptyResult);
    ...
    */
    public static function insert(&$inst,$add_fields=null,$db=null){
      ...
      //execute la requete
       ...
       $query .= " VALUES(";
       $query .= $db->parseValue($inst->guidId).",";
       $query .= $db->parseValue($inst->guidValue).",";
       if(is_array($add_fields))
    ...
    */
    public static function update(&$inst,$db=null){
      ...
      //execute la requete
       $query = "UPDATE guid SET";
       $query .= " guid_id =".$db->parseValue($inst->guidId).",";
       $query .= " guid_value =".$db->parseValue($inst->guidValue).",";
       $query = substr($query,0,-1);//remove last ','
       $query .= " where guid_id=".$db->parseValue($inst->guidId);
       if($db->execute($query,$result))
          return true;
    ...
    */
    public static function getByRelation(&$inst,$obj,$db=null){
        ...
        $objectTableName  = GuidMgr::nameToCode($objectName);
        ...
        if(is_string($obj->$objectIdName))
            $select = ("guid_id = (select guid_id from $objectTableName where ".$objectTableName."_id='".$obj->$objectIdName."')");
        else
            $select = ("guid_id = (select guid_id  from $objectTableName where ".$objectTableName."_id=".$obj->$objectIdName.")");

        return GuidMgr::get($inst,$select,$db);
    }
