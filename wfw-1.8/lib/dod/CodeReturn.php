...
class CodeReturn
{
   public function getId(){
      return $this->codeReturnId;
  }
   public function setId($id){
      return $this->codeReturnId = $id;
  }
...
class CodeReturnMgr
{
     ...
     * @param $inst Entity instance (CodeReturn)
     * @param $doc Parent document
     * @return New element node
     */
    public static function toXML(&$inst,$doc) {
        $node = $doc->createElement(strtolower("CodeReturn"));
        
        $node->appendChild($doc->createTextElement("code_return_id",$inst->codeReturnId));
    ...
    */
    public static function getAll(&$list,$cond,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from code_return where $cond";
      ...
      //extrait les instances
       ...
       while( $result->seek($i,iDatabaseQuery::Origin) ){
        $inst = new CodeReturn();
        CodeReturnMgr::bindResult($inst,$result);
    ...
    /*
      ...
      @param $inst CodeReturn instance pointer to initialize
      @param $cond SQL Select condition
      @param $db iDataBase derived instance
    */
    public static function bindResult(&$inst,$result){
          $inst->codeReturnId = $result->fetchValue("code_return_id");
    ...
    public static function get(&$inst,$cond,$db=null){
      ...
      //execute la requete
       ...
       if($db->execute($query,$result)){
          ...
          return CodeReturnMgr::bindResult($inst,$result);
       }
       return false;
    }
    ...
    */
    public static function getById(&$inst,$id,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from code_return where code_return_id=".$db->parseValue($id);
       if($db->execute($query,$result)){
            $inst = new CodeReturn();
             if(!$result->rowCount())
                 return RESULT(cResult::Failed,iDatabaseQuery::EmptyResult);
    ...
    */
    public static function insert(&$inst,$add_fields=null,$db=null){
       ...
       //id initialise ?
       if(!isset($inst->codeReturnId))
           return RESULT(cResult::Failed, cApplication::EntityMissingId);
      
      //execute la requete
       $query = "INSERT INTO code_return (";
       $query .= " code_return_id,";
       $query .= " description,";
       ...
       $query .= " VALUES(";
       $query .= $db->parseValue($inst->codeReturnId).",";
    ...
    */
    public static function update(&$inst,$db=null){
      ...
      //execute la requete
       $query = "UPDATE code_return SET";
       $query .= " code_return_id =".$db->parseValue($inst->codeReturnId).",";
       $query .= " description =".$db->parseValue($inst->description).",";
       $query = substr($query,0,-1);//remove last ','
       $query .= " where code_return_id=".$db->parseValue($inst->codeReturnId);
       if($db->execute($query,$result))
          return true;
    ...
    */
    public static function getByRelation(&$inst,$obj,$db=null){
        ...
        $objectTableName  = CodeReturnMgr::nameToCode($objectName);
        ...
        if(is_string($obj->$objectIdName))
            $select = ("code_return_id = (select code_return_id from $objectTableName where ".$objectTableName."_id='".$obj->$objectIdName."')");
        else
            $select = ("code_return_id = (select code_return_id  from $objectTableName where ".$objectTableName."_id=".$obj->$objectIdName.")");

        return CodeReturnMgr::get($inst,$select,$db);
    }
