...
class CodeClassMgr
{
     ...
     * @param $inst Entity instance (CodeClass)
     * @param $doc Parent document
     * @return New element node
     */
    public static function toXML(&$inst,$doc) {
        $node = $doc->createElement(strtolower("CodeClass"));
        
        $node->appendChild($doc->createTextElement("code_class_id",$inst->codeClassId));       
    ...
    */
    public static function getAll(&$list,$cond,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from code_class where $cond";
      ...
      //extrait les instances
       ...
       while( $result->seek($i,iDatabaseQuery::Origin) ){
        $inst = new CodeClass();
        CodeClassMgr::bindResult($inst,$result);
    ...
    /*
      ...
      @param $inst CodeClass instance pointer to initialize
      @param $cond SQL Select condition
      @param $db iDataBase derived instance
    */
    public static function bindResult(&$inst,$result){
          $inst->codeClassId = $result->fetchValue("code_class_id");          
    ...
    public static function get(&$inst,$cond,$db=null){
      ...
      //execute la requete
       ...
       if($db->execute($query,$result)){
          ...
          return CodeClassMgr::bindResult($inst,$result);
       }
       return false;
    }
    ...
    */
    public static function getById(&$inst,$id,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from code_class where code_class_id=".$db->parseValue($id);
       if($db->execute($query,$result)){
            $inst = new CodeClass();
             if(!$result->rowCount())
                 return RESULT(cResult::Failed,iDatabaseQuery::EmptyResult);
    ...
    */
    public static function insert(&$inst,$add_fields=null,$db=null){
      ...
      //execute la requete
       ...
       $query .= " VALUES(";
       $query .= $db->parseValue($inst->codeClassId).",";
       if(is_array($add_fields))
    ...
    */
    public static function update(&$inst,$db=null){
      ...
      //execute la requete
       $query = "UPDATE code_class SET";
       $query .= " code_class_id =".$db->parseValue($inst->codeClassId).",";
       $query = substr($query,0,-1);//remove last ','
       $query .= " where code_class_id=".$db->parseValue($inst->codeClassId);
       if($db->execute($query,$result))
          return true;
    ...
    */
    public static function getByRelation(&$inst,$obj,$db=null){
        ...
        $objectTableName  = CodeClassMgr::nameToCode($objectName);
        ...
        if(is_string($obj->$objectIdName))
            $select = ("code_class_id = (select code_class_id from $objectTableName where ".$objectTableName."_id='".$obj->$objectIdName."')");
        else
            $select = ("code_class_id = (select code_class_id  from $objectTableName where ".$objectTableName."_id=".$obj->$objectIdName.")");

        return CodeClassMgr::get($inst,$select,$db);
    }
