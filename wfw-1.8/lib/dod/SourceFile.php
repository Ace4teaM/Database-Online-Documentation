...
class SourceFileMgr
{
     ...
     * @param $inst Entity instance (SourceFile)
     * @param $doc Parent document
     * @return New element node
     */
    public static function toXML(&$inst,$doc) {
        $node = $doc->createElement(strtolower("SourceFile"));
        
        $node->appendChild($doc->createTextElement("source_file_id",$inst->sourceFileId));
    ...
    */
    public static function getAll(&$list,$cond,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from source_file where $cond";
      ...
      //extrait les instances
       ...
       while( $result->seek($i,iDatabaseQuery::Origin) ){
        $inst = new SourceFile();
        SourceFileMgr::bindResult($inst,$result);
    ...
    /*
      ...
      @param $inst SourceFile instance pointer to initialize
      @param $cond SQL Select condition
      @param $db iDataBase derived instance
    */
    public static function bindResult(&$inst,$result){
          $inst->sourceFileId = $result->fetchValue("source_file_id");
    ...
    public static function get(&$inst,$cond,$db=null){
      ...
      //execute la requete
       ...
       if($db->execute($query,$result)){
          ...
          return SourceFileMgr::bindResult($inst,$result);
       }
       return false;
    }
    ...
    */
    public static function getById(&$inst,$id,$db=null){
      ...
      //execute la requete
       $query = "SELECT * from source_file where source_file_id=".$db->parseValue($id);
       if($db->execute($query,$result)){
            $inst = new SourceFile();
             if(!$result->rowCount())
                 return RESULT(cResult::Failed,iDatabaseQuery::EmptyResult);
    ...
    */
    public static function insert(&$inst,$add_fields=null,$db=null){
      ...
      //execute la requete
       ...
       $query .= " VALUES(";
       $query .= $db->parseValue($inst->sourceFileId).",";
    ...
    */
    public static function update(&$inst,$db=null){
      ...
      //execute la requete
       ...
       $query .= " source_file_id =".$db->parseValue($inst->sourceFileId).",";
       $query .= " filename =".$db->parseValue($inst->filename).",";
       $query = substr($query,0,-1);//remove last ','
       $query .= " where source_file_id=".$db->parseValue($inst->sourceFileId);
       if($db->execute($query,$result))
          return true;
    ...
    */
    public static function getByRelation(&$inst,$obj,$db=null){
        ...
        $objectTableName  = SourceFileMgr::nameToCode($objectName);
        ...
        if(is_string($obj->$objectIdName))
            $select = ("source_file_id = (select source_file_id from $objectTableName where ".$objectTableName."_id='".$obj->$objectIdName."')");
        else
            $select = ("source_file_id = (select source_file_id  from $objectTableName where ".$objectTableName."_id=".$obj->$objectIdName.")");

        return SourceFileMgr::get($inst,$select,$db);
    }
